...
class hr_timesheet_lines_prisme(osv.osv):
    ...
    # Method overriden to do nothing (nothing to change in a working hour
    # when we change the quantity).
...
#        'partner_id': fields.related('account_id', 'partner_id', type='many2one', string='Partner', relation='res.partner', store=True, readonly=True),
#        'general_account_id': fields.many2one('account.account', 'General Account', required=True, readonly=True,ondelete='restrict'),
#        'journal_id': fields.many2one('account.analytic.journal', 'Analytic Journal', required=True, readonly=True,ondelete='restrict', select=True),
#        'invoice_id': fields.many2one('account.invoice', 'Invoice', ondelete="set null",readonly=True),
